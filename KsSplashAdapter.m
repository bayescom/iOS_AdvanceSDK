...
@interface KsSplashAdapter ()<KSAdSplashInteractDelegate>
...
@property (nonatomic, strong) KSAdSplashViewController *vc;
...
- (void)loadAd {
    ...
    [KSAdSplashManager setInteractDelegate:self];
    if (_supplier.state == AdvanceSdkSupplierStateSuccess) {// 并行请求保存的状态 再次轮到该渠道加载的时候 直接show
        ...
        [KSAdSplashManager setPosId:_supplier.adspotid];
...
- (void)showAd {
    ...
    self.vc.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
    WeakSelf(self);
    [[UIApplication sharedApplication].adv_getCurrentWindow.rootViewController presentViewController:self.vc animated:YES completion:^{
        StrongSelf(self);
    }];
}
