//
//  AdvYYKVStorage.h
//  AdvYYCache <https://github.com/ibireme/AdvYYCache>
//
...
/**
 AdvYYKVStorageItem is used by `AdvYYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface AdvYYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `AdvYYKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use AdvYYKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use AdvYYKVStorageTypeFile to get better performance.
 * You can use AdvYYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, AdvYYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    AdvYYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    AdvYYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    AdvYYKVStorageTypeMixed = 2,
};



/**
 AdvYYKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for AdvYYKVStorage is `initWithPath:type:`. 
 ...
 */
@interface AdvYYKVStorage : NSObject
...
@property (nonatomic, readonly) AdvYYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(AdvYYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is AdvYYKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is AdvYYKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is AdvYYKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(AdvYYKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 AdvYYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is AdvYYKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is AdvYYKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is AdvYYKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable AdvYYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable AdvYYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `AdvYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<AdvYYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `AdvYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<AdvYYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
